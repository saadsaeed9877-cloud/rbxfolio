...
/**
 * Shell - Main layout wrapper with header, nav, and footer
 */
    ...
    <div className="min-h-screen bg-[#070a08] text-[#f4f7f4]">
      ...
      <footer className="border-t border-white/8 px-5 py-10">
        <div className="mx-auto flex max-w-[1380px] flex-col gap-5 text-sm text-white/35 sm:flex-row sm:items-center sm:justify-between">
          <span className="font-display font-bold text-white">RBXFOLIO</span>
...
/**
 * SectionTitle - Reusable section header with eyebrow, title, copy, and action
 */
export function SectionTitle({
  ...
  return (
    <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        {eyebrow && (
          <div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-[.18em] text-[#b7ff3c]">
            <span className="h-px w-6 bg-[#b7ff3c]" />
            {eyebrow}
          </div>
        )}
        <h2 className="font-display text-3xl font-bold tracking-[-.04em] md:text-4xl">
          {title}
        </h2>
        {copy && (
      ...
      {action && (
        <button
          ...
          className="flex items-center gap-2 text-sm font-semibold text-[#b7ff3c] hover:text-[#c6ff65]"
        >
          {action}
...
/**
 * CreatorCard - Card displaying creator profile
 */
    ...
    <article className="group rounded-2xl border border-white/8 bg-[#0c110d] p-5 transition-all hover:-translate-y-1 hover:border-[#b7ff3c]/30">
      <div className="flex items-start justify-between">
        <Link
          ...
          className={`grid size-14 place-items-center rounded-2xl ${creator.color} font-display text-sm font-black text-black shadow-lg`}
        >
          {creator.initials}
        </Link>
        <button
      ...
      <Link href={`/u/${creator.handle}`}>
        <h3 className="mt-5 font-display text-lg font-bold">{creator.name}</h3>
      </Link>
      ...
      <div className="mt-4 flex flex-wrap gap-1.5">
        {creator.skills.map((s) => (
          <span
            key={s}
            className="rounded-md border border-white/8 bg-white/[.03] px-2 py-1 text-[10px] text-white/50"
          >
            {s}
          </span>
        ))}
      ...
      <div className="mt-5 flex items-center justify-between border-t border-white/8 pt-4 text-xs">
        <span className="text-white/35">
          <strong className="text-white/80">{creator.followers}</strong> followers
        </span>
...
/**
 * ProjectCard - Card displaying project with image
 */
export function ProjectCard({
  ...
  return (
    <article
      className={`group overflow-hidden rounded-2xl border border-white/8 bg-[#0c110d] ${
        large ? 'md:col-span-2' : ''
      }`}
    >
      <div
      ...
      >
        <img
          ...
          className="h-full w-full object-cover saturate-[.75] transition duration-700 group-hover:scale-105 group-hover:saturate-100"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
        <span className="absolute left-4 top-4 rounded-md border border-white/15 bg-black/55 px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest backdrop-blur-md">
          {project.role}
        </span>
        <button
          onClick={() => setSaved(!saved)}
          aria-label="Save project"
          className={`absolute right-4 top-4 grid size-9 place-items-center rounded-lg border backdrop-blur-md ${
            saved
...
/**
 * PageHero - Hero section for pages with kicker, title, and copy
 */
  ...
  return (
    <section className="border-b border-white/8 px-5 py-14 md:py-20">
      <div className="mx-auto max-w-[1380px]">
        <p className="mb-4 text-xs font-bold uppercase tracking-[.2em] text-[#b7ff3c]">
          {kicker}
        </p>
        <h1 className="font-display max-w-4xl text-5xl font-bold leading-[.95] tracking-[-.055em] md:text-7xl">
          {title}
        </h1>
...
/**
 * Role icons mapping for filters and tags
 */
